...
def pretty_nl(name: str, args: list[str]) -> str:
  ...
  if name in ['coll', 'C']:
    return '' + ','.join(args) + ' thẳng hàng'
  if name == 'collx':
    return '' + ','.join(list(set(args))) + ' thẳng hàng'
  if name in ['cyclic', 'O']:
    return '' + ','.join(args) + ' cùng nằm trên một đường tròn'
  if name in ['midp', 'midpoint', 'M']:
    x, a, b = args
    return f'{x} là trung điểm của {a}{b}'
  ...
  if name in ['contri2', 'contri', 'contri*']:
    a, b, c, x, y, z = args
    return f'\u0394{a}{b}{c} đồng dạng với \u0394{x}{y}{z}'
  if name in ['circle', 'I']:
    o, a, b, c = args
    return f'{o} là trọng tâm của \\Delta {a}{b}{c}'
  if name == 'foot':
    a, b, c, d = args
    return f'{a} chân đường {b} xuống {c}{d}'
